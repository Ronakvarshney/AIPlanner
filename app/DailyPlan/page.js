...
const DailyPlan = ({ trip }) => {
  // ✅ Prevent errors by ensuring `trip` is defined and has at least one element
  if (!trip || trip.length === 0 || !trip[0]?.travelPlan?.itinerary) {
    return <p className="text-center text-red-500">No trip data available.</p>;
  }

  return (
    <div className="p-4">
      <h2 className="font-bold text-lg mb-4 text-center md:text-left">
        Places to Visit
      </h2>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              ...
              <div className="flex flex-col gap-2 text-center sm:text-left">
                ...
                <h2 className="text-sm text-red-600">
                  TicketPrice - {item.ticketPricing || "N/A"}
                </h2>
                <h2 className="text-sm font-medium text-green-700">
                  Travel timing - ⏱️ {item.travelTime || "N/A"}
                </h2>

                <a
                  href={`https://www.google.com/maps/search/?api=1&query=${
                    trip[0]?.location + " " + item.placeName
                  }`}
                  target="_blank"
